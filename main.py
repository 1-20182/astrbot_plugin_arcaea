...
@register("astrbot_plugin_arcaea_sticker", "YourName", "Arcaea 表情包生成器（智能单双文字）", "4.1.0")
class ArcaeaStickerPlugin(Star):
    ...
    def __init__(self, context: Context, config: dict = None):
        ...
        self.simple_font_size = int(config.get("simple_font_size", 35))
        self.simple_left_rotation = float(config.get("simple_left_rotation", -45))
        self.simple_right_rotation = float(config.get("simple_right_rotation", 45))
        self.simple_left_margin = int(config.get("simple_left_margin", 20))
        self.simple_top_margin = int(config.get("simple_top_margin", 20))
        self.simple_right_margin = int(config.get("simple_right_margin", self.simple_left_margin))

        # 触发关键词（用于随机表情包）
        keywords_str = config.get("trigger_keywords", "arc表情包")
        self.trigger_keywords = [kw.strip() for kw in keywords_str.split(",") if kw.strip()]

        self.generator = ArcaeaStickerGenerator(self.resource_dir, custom_font=custom_font)
        ...
        logger.info(f"Arcaea Sticker 插件 v4.1.0 加载，可用角色：{', '.join(self.available_characters)}")
