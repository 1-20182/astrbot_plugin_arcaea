...
class ArcaeaStickerGenerator:
    def __init__(self, resource_dir: str, custom_font: str = ""):
    ...
    def _darken_color(self, rgb, factor=0.7):
    ...
    def _draw_text_on_image(self, image: Image.Image, text: str, center_x: int, center_y: int,
                            rotate: float, font_path: str, size: int, color, stroke_width=0):
        ...
        if stroke_width > 0:
            text_draw.text((-bbox[0], -bbox[1]), text, fill=color, font=font,
                           stroke_width=stroke_width, stroke_fill=self._darken_color(color, 0.7))
        else:
            text_draw.text((-bbox[0], -bbox[1]), text, fill=color, font=font)
